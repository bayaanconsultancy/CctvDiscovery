    ...
    private JButton createCancelButton() {
        JButton button = new JButton("Cancel Discovery") {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (isEnabled()) {
                    GradientPaint gp = new GradientPaint(0, 0, new Color(220, 53, 69), 0, getHeight(), new Color(201, 48, 44));
                    g2d.setPaint(gp);
                } else {
    ...
    public void setProgress(int current, int total) {
        SwingUtilities.invokeLater(() -> {
            if (total > 0) {
                ...
                countLabel.setText(current + " of " + total + " cameras processed");
            } else {
                progressBar.setIndeterminate(true);
    ...
    public void updateCurrentCamera(String camera, String status) {
        SwingUtilities.invokeLater(() -> {
            if (camera != null && !camera.isEmpty()) {
                currentCameraLabel.setText("Processing: " + camera + " - " + status);
            } else {
