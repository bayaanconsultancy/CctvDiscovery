...
public class NetworkSelectionPanel extends JPanel {
    ...
    private void startScan(WizardFrame frame) {
        ...
        if (!startIp.isEmpty() || !endIp.isEmpty()) {
            if (startIp.isEmpty() || endIp.isEmpty()) {
                JOptionPane.showMessageDialog(frame, "Both start and end IP addresses are required", "Validation Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (!IpRangeValidator.isValidIp(startIp) || !IpRangeValidator.isValidIp(endIp)) {
                JOptionPane.showMessageDialog(frame, "Invalid IP address format", "Validation Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
        } else if (networkCombo.getSelectedItem() == null) {
            JOptionPane.showMessageDialog(frame, "Please select a network interface or enter IP range",
                    "Validation Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        scanButton.setEnabled(false);
        startIpField.setEnabled(false);
        endIpField.setEnabled(false);
        ...
        new SwingWorker<List<Camera>, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    JOptionPane.showMessageDialog(frame, "Discovery failed: " + ex.getMessage(), "Error",
                            JOptionPane.ERROR_MESSAGE);
                } finally {
                    scanButton.setEnabled(true);
                    startIpField.setEnabled(true);
                    endIpField.setEnabled(true);
                }
            }
        }.execute();
    }
    ...
    private void validateInputs() {
        ...
        if (manualEntry) {
            ...
            if (endIp.isEmpty()) {
                endIpField.setBorder(DEFAULT_BORDER);
                if (isValid)
                    validationLabel.setText("End IP is required"); // Only show if start is valid
                isValid = false;
