    ...
    private static void createCameraSheet(Sheet sheet, List<Camera> cameras, boolean maskPasswords, Workbook workbook) {
            ...
            if (!maskPasswords) {
                ...
                warningCell.setCellValue(
                        "⚠️ WARNING: This file contains PLAINTEXT PASSWORDS. Store securely and delete when no longer needed.");
            ...
            String[] headers = { "IP Address", "MAC Address", "Manufacturer", "Model", "Camera Name", "Serial Number",
                    "Firmware", "Time Diff (sec)",
                    "Username", "Password",
            ...
            for (Camera camera : cameras) {
                ...
                createStyledCell(row, 1, camera.getMacAddress() != null ? camera.getMacAddress() : "", defaultStyle);
                createStyledCell(row, 2, camera.getManufacturer() != null ? camera.getManufacturer() : "",
                        defaultStyle);
                createStyledCell(row, 3, camera.getModel() != null ? camera.getModel() : "", defaultStyle);
                createStyledCell(row, 4, camera.getCameraName() != null ? camera.getCameraName() : "", defaultStyle);
                createStyledCell(row, 5, camera.getSerialNumber() != null ? camera.getSerialNumber() : "",
                        defaultStyle);
                createStyledCell(row, 6, camera.getFirmwareVersion() != null ? camera.getFirmwareVersion() : "",
                        defaultStyle);
                createStyledCell(row, 7, String.valueOf(camera.getTimeDifferenceMs() / 1000), defaultStyle);
                createStyledCell(row, 8, camera.getUsername() != null ? camera.getUsername() : "", defaultStyle);
                String password = camera.getPassword() != null ? camera.getPassword() : "";
                if (maskPasswords && !password.isEmpty()) {
                    password = "****";
                }
                createStyledCell(row, 9, password, defaultStyle);

                StreamInfo main = camera.getMainStream();
                if (main != null) {
                    createStyledCell(row, 10, main.getRtspUrl() != null ? main.getRtspUrl() : "", defaultStyle);
                    createStyledCell(row, 11, main.getResolution() != null ? main.getResolution() : "", defaultStyle);
                    createStyledCell(row, 12, main.getCodec() != null ? main.getCodec() : "", defaultStyle);
                    createStyledCell(row, 13, String.valueOf(main.getBitrate()), defaultStyle);
                    createStyledCell(row, 14, String.valueOf((int) main.getFps()), defaultStyle);
                    if (main.getError() != null) {
                        String currentError = camera.getErrorMessage();
                        camera.setErrorMessage(currentError != null ? currentError + "; Main: " + main.getError()
                                : "Main: " + main.getError());
                    }
                }
                ...
                if (sub != null) {
                    createStyledCell(row, 15, sub.getRtspUrl() != null ? sub.getRtspUrl() : "", defaultStyle);

                    Cell resCell = createStyledCell(row, 16, sub.getResolution() != null ? sub.getResolution() : "",
                            defaultStyle);
                    if (!isValidSubResolution(sub.getResolution())) {
                        resCell.setCellStyle(redStyle);
                    }

                    Cell codecCell = createStyledCell(row, 17, sub.getCodec() != null ? sub.getCodec() : "",
                            defaultStyle);
                    if (!isH264(sub.getCodec())) {
                        codecCell.setCellStyle(redStyle);
                    }

                    createStyledCell(row, 18, String.valueOf(sub.getBitrate()), defaultStyle);
                    createStyledCell(row, 19, String.valueOf((int) sub.getFps()), defaultStyle);
                    if (sub.getError() != null) {
                        String currentError = camera.getErrorMessage();
                        camera.setErrorMessage(currentError != null ? currentError + "; Sub: " + sub.getError()
                                : "Sub: " + sub.getError());
                    }
                }

                createStyledCell(row, 20, camera.getErrorMessage() != null ? camera.getErrorMessage() : "",
                        defaultStyle);
            }
